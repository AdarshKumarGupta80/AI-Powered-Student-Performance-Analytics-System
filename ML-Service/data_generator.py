...
def generate_training_data(n_samples: int = 2000, seed: int = 42) -> pd.DataFrame:
    ...
    for _ in range(n_samples):
        ...
        recent_avg = np.clip(
            avg_score + np.random.normal(0, 8),
            10,
            100
        )

        trend = np.clip(
            (recent_avg - avg_score) + np.random.normal(0, 5),
            -40,
            40
        )
        ...
        assign_completion = np.clip(
            np.random.normal(78, 20),
            0,
            100
        )
        ...
        assign_avg_score = np.clip(
            avg_score + np.random.normal(2, 8),
            0,
            100
        )
        ...
        lecture_completion = np.clip(
            np.random.normal(0.70, 0.20),
            0,
            1
        )
        ...
        final_score = np.clip(
            ...
            trend * 0.10 +
            attendance * 0.10 +
            assign_completion * 0.08 +
            weekly_hours * 0.50 +
            consistency_index * 0.05 -
            consec_absences * 4 -
            (subject_variance * 0.08) +
