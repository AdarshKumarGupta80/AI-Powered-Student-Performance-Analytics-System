...
export default function AppShell({ children }) {
  ...
  const roleLabel = useMemo(() => {
    ...
    if (user?.role === 'TEACHER') return 'Teacher analytics workspace';
    if (user?.role === 'PARENT') return 'Parent progress view';
        ...
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-slate-50/85 backdrop-blur dark:border-slate-800 dark:bg-slate-950/85">
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6 lg:px-8">
            ...
            <div className="ml-auto flex items-center gap-2">
              <p className="hidden text-sm font-medium text-slate-500 dark:text-slate-400 md:block">{roleLabel}</p>
